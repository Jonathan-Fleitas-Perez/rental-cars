...
export  function OurFleet() {
  return (
    <div className="max-w-6xl mx-auto text-center py-12 lg:py-40 p-6">
        ...
        <p className="text-xl mt-2 lg:mt-5 lg:text-xl text-center w-full mx-auto max-w-2xl mb-5 lg:mb-10">Dont deny yourself of driving the best premium cars from around the world here and now the world</p>
        ...
        <div className="mb-10">
            <div className="grid grid-cols-3 gap-x-6 mb-6">
                {dataFirstBlockOurFleet.map(({url})=>(
                    <div key={url}>
                        <Image src={`/images/OurFleetCars/${url}`} alt="Car" width={400} height={300} className="rounded-xl"/>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-4 gap-x-6 max-w-5xl mx-auto">
                {dataSecondBlockOurFleet.map(({url})=>(
                    <div key={url}>
                        <Image src={`/images/OurFleetCars/${url}`} alt="Car" width={400} height={300} className="rounded-xl aspect-[3/2]"/>
                    </div>
                ))}
            </div>
        </div>
